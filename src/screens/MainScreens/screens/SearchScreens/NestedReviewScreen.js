...
export default function NestedReviewScreen({navigation, route}) {
  ...
  useEffect(() => {
    if (route.params.item.id) {
      if (route.params.responce4) {
        setItem(route.params.responce4);
      } else if (route.params.responce3) {
        setItem(route.params.responce3);
      } else if (route.params.responce2) {
        setItem(route.params.responce2);
      } else {
        setItem(route.params.responce1);
      }
      getResponseData();
    }
  ...
  const getResponseData = () => {
    let responseUserId;
    if (route.params.responce4) {
      responseUserId = route.params.responce4.id;
    } else if (route.params.responce3) {
      responseUserId = route.params.responce3.id;
    } else if (route.params.responce2) {
      responseUserId = route.params.responce2.id;
    } else {
      responseUserId = route.params.responce1.id;
    }
    let paramsResponce = {
      reviewId: route.params.item.id,
      responseId: responseUserId,
      start: counter,
      limit: 10,
    };
            ...
            <View style={styles.cmntDisplayBox}>
              <View
                ...
                <TouchableOpacity
                  ...
                  <Text
                    numberOfLines={1}
                    style={[styles.reviewTagname, {width: '40%'}]}>
              ...
              <TouchableOpacity
                ...
                disabled={data.responseLevel > 4 ? true : false}
                onPress={() =>
                  navigation.push(Constants.screen_nested_review, {
                    item: route.params.item,
                    responce1: route.params.responce1,
                    responce2: route.params.responce2
                      ? route.params.responce2
                      : data,
                    responce3: route.params.responce2
                      ? route.params.responce3
                        ? route.params.responce3
                        : data
                      : null,
                    responce4: route.params.responce3 ? data : null,
                  })
                }>
          ...
          <View style={{flexDirection: 'row', paddingHorizontal: RFValue(5)}}>
            ...
            <View style={{marginHorizontal: RFValue(8), width: '92%'}}>
                    ...
                    navigation.push(Constants.screen_nested_review, {
                      ...
                      responce3: route.params.responce2
                        ? route.params.responce3
                          ...
                          : data
                        : nestedRes,
                      responce4: route.params.responce2
                        ? route.params.responce3
                          ? data
                          : nestedRes
                        : null,
              ...
              <View
                ...
                {isLoggedIn && nestedRes.responseLevel < 4 && (
                  <TouchableOpacity
                    style={{flex: 1}}
                    onPress={() =>
                      userGetData(nestedRes, nestedRes?.byProfileTagName)
                    }>
                    <AddResponses width={RFValue(21)} height={RFValue(17)} />
                  </TouchableOpacity>
                )}
              ...
              {data.responseCount > 1 && (
                <TouchableOpacity
                  ...
                  style={[styles.perentContainer, {marginBottom: RFValue(8)}]}>
                  <View
                    style={{
      ...
      <>
        ...
        {route.params.responce2 && (
          <ReviewComponents
            ...
            shareReviewHide={true}
            item={route.params.responce1}
          />
        )}
        {route.params.responce3 && (
          <ReviewComponents
            ...
            shareReviewHide={true}
            item={route.params.responce2}
          />
        )}
        {route.params.responce4 && (
          <ReviewComponents
            isLoggedIn={isLoggedIn}
            nestedReview={true}
            onClickResponse={() => {
              navigation.push(Constants.screen_nested_review, {
                ...
                responce1: route.params.responce1,
                responce2: route.params.responce2,
                responce3: route.params.responce3,
              });
            }}
            shareReviewHide={true}
            item={route.params.responce3}
          />
        )}
        <View
          style={{paddingHorizontal: RFValue(15), paddingBottom: RFValue(15)}}>
          ...
          <View style={{marginTop: RFValue(10), marginLeft: RFValue(3)}}>
          ...
          {item.responseLevel < 4 && (
            <>
              ...
              <View style={styles.addResponseIcnContainer}>
                ...
                {isLoggedIn && item.responseLevel < 4 && (
                  <TouchableOpacity
                    style={{flex: 1}}
                    onPress={createPageModalShow}>
                    <AddResponses width={RFValue(21)} height={RFValue(17)} />
  ...
  const sharePost = async (revId, resId) => {
    const url = await buildLink(revId, resId);
    const title = 'CitizeX';
  ...
  const buildLink = async (revId, resId) => {
    const link = await dynamicLinks().buildShortLink(
      {
        link: `https://keeku.page.link/app?post_id=${revId}&response_id=${resId}`,
        ...
        <ActionSheet
          ...
          actionItems={[
            ...
            },
            {
              ...
              textStyle: [styles.txtStyle],
              onPress: async () => {
                await sharePost(route.params.item.id, item?.id);
              },
            },
          ]}
        />
